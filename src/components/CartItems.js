import React, { useState, useEffect } from 'react'
import { View, Text, Image, ScrollView } from 'react-native'
import FontAwesome from 'react-native-vector-icons/FontAwesome'
import { Divider, } from 'react-native-paper'


function CartItems({ navigation }) {

    const [isQuantity, setIsQuantity] = useState(0)



    const incrementQty = () => {
        const value = isQuantity
        setIsQuantity(value + 1)
    }

    const decrementQty = () => {
        if (isQuantity - 1 > -1) {
            const value = isQuantity
            setIsQuantity(value - 1)
        }
    }

    const pic = 'data:image/png;base64,'

    return (
        <View style={{ flex: 1, backgroundColor: '#fff' }}>


            <View style={{ flex: 1, flexDirection: 'row', borderRadius: 5, }}>
                <View style={{ flex: 0.8, alignItems: 'center', justifyContent: 'center', paddingVertical: 10 }}>
                    <Image source={{ uri: pic }} style={{ width: 70, height: 70, borderRadius: 50 }} />
                </View>
                <View style={{ flex: 1, paddingVertical: 10 }}>
                    <Text style={{ fontSize: 20, fontWeight: 'bold' }}>Tomatoes</Text>
                    <Text style={{ fontWeight: 'bold', fontSize: 15, paddingLeft: 5 }}>{'\u20B9'} 30/kg</Text>
                    <Text style={{ fontSize: 15, color: '#888', fontWeight: 'bold', paddingLeft: 5 }}>Total : {'\u20B9'} 60</Text>
                </View>
                <View style={{ flex: 0.8, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', justifyContent: 'space-evenly', paddingVertical: 10 }}>
                    <FontAwesome
                        name="minus-square"
                        size={25}
                        color='#DC3545'
                        onPress={() => decrementQty()}
                    />
                    <Text style={{ alignSelf: 'center', fontWeight: 'bold', fontSize: 17 }}>{isQuantity}</Text>
                    <FontAwesome
                        name='plus-square'
                        size={25}
                        color='#28A745'
                        onPress={() => incrementQty()}
                    />
                </View>

            </View>
            <Divider style={{ backgroundColor: '#000' }} />
        </View>
    )
}

export default CartItems
